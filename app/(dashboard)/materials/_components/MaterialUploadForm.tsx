...
export function MaterialUploadForm({ subjectId, onSuccess }: MaterialUploadFormProps) {
  ...
  async function handleLink(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/materials", {
        ...
        body: JSON.stringify({ subjectId, title: linkTitle, type: "LINK", url: linkUrl }),
      });
  ...
  async function handleNote(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/materials", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ subjectId, title: noteTitle, type: "NOTE", content: noteContent }),
      });
      if (!res.ok) throw new Error();
      ...
      {tab === "link" && (
        <form onSubmit={handleLink} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div>
            ...
            <input id="material-link-url" className="input" type="url" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} placeholder="https://" required />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: "var(--text-muted)", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>
              Title
            </label>
            <input id="material-link-title" className="input" value={linkTitle} onChange={(e) => setLinkTitle(e.target.value)} placeholder="e.g. Khan Academy — Integration" required />
          </div>
          <Button type="submit" loading={loading} style={{ alignSelf: "flex-start" }}>
            Add Link
          </Button>
      ...
      {tab === "note" && (
        <form onSubmit={handleNote} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div>
            ...
            <input id="material-note-title" className="input" value={noteTitle} onChange={(e) => setNoteTitle(e.target.value)} placeholder="e.g. Key Formulas — Chapter 4" required />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: "var(--text-muted)", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>
              Content
